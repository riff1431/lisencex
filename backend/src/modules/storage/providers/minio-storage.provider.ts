...
export interface MinioProviderConfig {
  endpoint: string; // e.g. http://minio.internal:9000
  accessKeyId: string;
...
/**
 * MinIO storage provider — MinIO exposes the S3 API, so this uses the AWS SDK
 * S3 client pointed at the MinIO endpoint with path-style addressing
 * (forcePathStyle: true), which MinIO requires.
 */
@Injectable()
export class MinioStorageProvider implements StorageProvider {
  ...
  setConfig(config: MinioProviderConfig) {
    this.config = config;
  ...
  private getClient(): S3Client {
    if (!this.client) {
      throw new BadRequestException('MinIO storage provider is not properly configured');
    }
    return this.client;
  }
  ...
  async delete(path: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`MinIO delete failed for key: ${path}`, err);
      return false;
    }
  }
  ...
  async download(path: string): Promise<Buffer> {
    ...
    if (!response.Body) {
      throw new BadRequestException(`Empty response body from MinIO for: ${path}`);
    }
  ...
  async copy(sourcePath: string, destPath: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`MinIO copy failed from ${sourcePath} to ${destPath}`, err);
      return false;
    }
  }
    ...
    try {
      ...
      await client.send(
        new PutObjectCommand({
          Bucket: this.config.bucket,
          Key: testKey,
          Body: Buffer.from('LicenseNest MinIO Connection Probe'),
      ...
      return {
        success: true,
        message: `Successfully connected to MinIO bucket "${this.config.bucket}" at ${this.config.endpoint}`,
